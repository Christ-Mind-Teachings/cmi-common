...
import {gs, __lang} from "../_language/lang";
...
function validateLink(pid, link) {
  let rawLink;
  let pKey = g_sourceInfo.keyInfo.genParagraphKey(pid);

  try {
    rawLink = JSON.parse(link);
  }
  catch(error) {
    notify.error(gs("annotate:m3", "Invalid link; invalid format, get link from bookmark popup."));
    return false;
  }

  if (!rawLink.aid || !rawLink.desc || !rawLink.key || !rawLink.userId) {
    notify.error(gs("annotate:m4", "Invalid link; invalid format, get link from bookmark popup."));
    return false;
  }

  if (rawLink.key === pKey) {
    notify.error(gs("annotate:m5", "Invalid link; it references itself."));
    return false;
  }
...
function generateHorizontalList(listArray) {
  if (!listArray || listArray.length === 0) {
    return `
      <div class="item">
        <em>${gs("annotate:m6", "Annotation has no topics")}</em>
      </div>
    `;
  }
...
function generateComment(comment) {
  if (!comment) {
    return gs("annotate:m7", "No comment");
  }
  else {
    return comment;
  }
...
function bookmarkNavigatorActive() {
  if ($(".transcript").hasClass("bookmark-navigator-active")) {
    notify.info(gs("annotate:m10", "Annotation is disabled when the bookmark navigator is active."));
    return true;
  }
  return false;
}
...
function hoverNoteHandler() {
  $(".transcript").on("mouseenter", ".has-annotation", function(e) {
    ...
    $(".annotation-information .range").html(`${gs("label:range", "Range")}: ${annotation.rangeStart}/${annotation.rangeEnd}`);
...
function createBookmarkLink(pid, aid) {
  let pKey = g_sourceInfo.keyInfo.genParagraphKey(pid);
  let keyInfo = g_sourceInfo.keyInfo.describeKey(pKey);
    ...
    if (userInfo.userId === "xxx") {
      header2 = `
        <h4 class="ui left floated header">
          <i title="${gs("annotate:m11", "Sign into your account to share this bookmark to FB by email or to copy a link.")}" class="red window close outline icon"></i>
          <div class="content">
            ${formData.Comment}
          </div>
        </h4>
        <h4 class="ui right floated header">
          <i title="${gs("action:close", "Close Window" )}" class="share-annotation window close icon"></i>
        </h4>
      `;
    }
    else {
      header2 = `
        <h4 class="ui left floated header">
          <i title="${gs("action:fbshare", "Share to Facebook")}" class="share-annotation facebook icon"></i>
          <i title="${gs("action:emailshare", "Share via email")}" class="share-annotation envelope outline icon"></i>
          <i data-clipboard-text="${url}" title="${gs("action:cp2clip", "Copy Url to Clipboard")}" class="share-annotation linkify icon"></i>
          <div class="content">
            ${formData.Comment}
          </div>
        </h4>
...
*/
export function initialize(si) {
  if (si) {
    g_sourceInfo = si;
  }
  else {
    g_sourceInfo = getTeachingInfo();
  }
...
function makeTopicSelect(topics) {
  return (`
    <select name="topicList" id="annotation-topic-list" multiple="" class="search ui dropdown">
      <option value="">${gs("label:selecttopic", "Select Topic(s)")}</option>
...
function getTopicList(pid, aid, data) {
  ...
  net.fetchTopics()
    ...
    .catch(( error ) => {
      ...
      notify.error(`${gs("annotate:m12", "Unable to fetch bookmark topic list")}: ${error}`);
    });
}
