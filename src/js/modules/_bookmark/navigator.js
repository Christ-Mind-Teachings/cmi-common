...
function generateHorizontalList(listArray) {
  if (!listArray || listArray.length === 0) {
    return gs("annotate:m13", "No Topics" );
  }
        ...
        <div class="content">
          ...
          <div class="description">
            <button class="annotation-item ui icon basic button" data-range="${annotation.rangeStart}/${annotation.rangeEnd}" data-aid="${annotation.aid}" data-creationDate="${annotation.creationDate}" >
              ...
              ${annotation.Comment?annotation.Comment:gs("annotate:m7", "No comment" )}
            </button>
            <!--
...
function getCurrentBookmark(pageKey, actualPid, allBookmarks, bmModal, whoCalled) {
  ...
  let html = generateBookmark(actualPid, paragraphBookmarks, topics, gs("label:para", "Paragraph"));
  ...
  let resp = gs("action:prev", "Previous");
  if (!prevActualPid) {
...
function bookmarkManager(actualPid) {
  let sourceId = g_sourceInfo.keyInfo.getSourceId();
  let pageKey = g_sourceInfo.keyInfo.genPageKey().toString(10);
  let bmList = g_sourceInfo.getValue("bmList");
  let bmModal = g_sourceInfo.getValue("bmModal");
    ...
    getNextPrevUrl(pageKey, bmList, bmModal, "")
      .then((responses) => {
        ...
        if (!getCurrentBookmark(pageKey, actualPid, bmList, bmModal, "bookmark")) {
          notify.info(__lang`${"fragment:f1:A bookmark at"} ${actualPid} ${"fragment:f2:was not found"}`);
          return;
        }
...
export function initShareDialog(source) {
  ...
  $(".transcript").on("click", ".selected-annotation-wrapper .share-annotation", function(e) {
    ...
    if (!userInfo) {
      notify.info(gs("annotate:m14", "You must be signed in to share bookmarks."));
      return;
    }
    ...
    if ($(this).hasClass("facebook")) {
      ...
      if (pos > -1) {
        ...
        notify.error(gs("error1:e5", "Sorry, there was a problem, refresh the page and try again."));
        return;
      }
    ...
    if (userInfo.userId === "xxx") {
      ...
      header = __lang`
        <h4 class="ui header">
          ...
          <i data-clipboard-text="${url}" title="${"action:cp2clip:Copy Url to Clipboard"}" class="share-annotation linkify icon"></i>
          <div class="content">
            ${$(this).text()}
          </div>
        </h4>
...
*/
export function initNavigator(actualPid, si) {
  g_sourceInfo = si;
  bookmarkManager(actualPid);
}

export function initTopicNavigator(topicInfo, si) {
  g_sourceInfo = si;
  bookmarkTopicManager(topicInfo, si);
}
