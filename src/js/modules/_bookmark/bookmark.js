...
function bookmarkFeatureHandler() {
  $("#bookmark-toggle-disable-selection").on("click", function(e, show) {
    ...
    if (el.hasClass("disable-selection") && el.hasClass("user")) {
      let value = gs("menu:m1", "Disable Bookmark Creation");
      ...
      let value = gs("menu:m2", "Enable Bookmark Creation");
...
function initTranscriptPage(sharePid, si) {
  ...
  if (!topicInfo) {
    initBookmarkFeature(sharePid, si);
  }

  //setup bookmark navigator if requested
  let pid = showBookmark();
  if (pid) {
    initNavigator(pid, si);
  }
}

export function initBookmarkFeature(sharePid, si) {
