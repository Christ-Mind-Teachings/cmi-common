...
function processSelection(selection) {
  ...
  if (range.commonAncestorContainer.nodeName === "DIV") {
    notify.info(gs("error1:e8", "Please limit selected text to a single paragraph"));
  ...
  if (range.startContainer.parentElement.localName === "span") {
    notify.info(gs("error1:e6", "Don't include the paragraph number in your selection, please try again."));
  ...
  if (selectedText) {
    ...
    for (let ht of highlightedText) {
      if (selection.containsNode(ht, true)) {
        notify.info(gs("error1:e7", "Your selection is overlapping with another; overlapping is not supported."));
