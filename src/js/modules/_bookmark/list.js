  ...
  if (bkmk.length === 0) {
    return `
      <div class="bookmark-item item"> <!-- ${pid} -->
        ...
        <div class="content">
          <div class="header">
            <a href="${url}?bkmk=${pid}">${gs("label:para", "Paragraph" )}: ${pid}</a>
          </div>
        </div>
      </div> <!-- item: ${pid} -->
    `;
  ...
  */
  ...
  for (let pageKey in bookmarks) {
    if (pageKey !== "lastFetchDate" && pageKey !== "lastBuildDate") {
      info.push(g_sourceInfo.getPageInfo(pageKey, bookmarks[pageKey]));
    }
  }
...
async function queryBookmarks(key) {
  ...
  try {
    ...
    console.error(err);
    notify.error(gs("error1:e4", "Failed to get bookmarks"));
  }
}
...
*/
function initList() {
  const {sourceId} = g_sourceInfo.keyInfo.getKeyInfo();
