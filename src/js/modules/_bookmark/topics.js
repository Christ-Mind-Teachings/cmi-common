...
function formatTopic(topic) {
  if (topic === "__reset__") {
    return `<div class='reset-filter item'>${gs("label:clearfilter", "Clear Filter")}</div>`;
  }
...
function makeTopicList() {
  ...
  if (topics.length === 0) {
    return `<div class='ntf item'>${gs("annotate:m15", "Page has no topics")}</div>`;
  }
  topics.sort();
...
function topicSelectHandler() {
  $("#topic-menu-item").on("click", "#topic-menu-select > .item", function(e) {
    ...
    if ($(this).hasClass("reset-filter")) {
      ...
      //reset header text
      $("#topic-menu-item").prev(".header").text(`${gs("label:topicfilter", "Topic Filter")}: None`);
    ...
    $("#topic-menu-item").prev(".header").html(`${gs("label:topicfilter", "Topic Filter")}: <span class="red">${topic}</span>`);
...
function filterResetHandler() {
  ...
  $(".page-topics-filter-reset").on("click", function(e) {
    ...
    $("#current-topic-filter").html(`${gs("label:topicfilter", "Topic Filter")}: None`);
