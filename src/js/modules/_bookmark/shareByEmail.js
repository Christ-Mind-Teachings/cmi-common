...
export function initShareByEmail(si) {
  ...
  $("form[name='emailshare']").on("submit", async function(e) {
    ...
    if (!userInfo) {
      notify.warning(gs("annotate:m14", "You must be signed in to share bookmarks."));
    ...
    if (formData.mailList.length === 0 && formData.emailAddresses.length === 0) {
      notify.info(gs("error1:e9", "Please enter at least one email address."));
      return;
    }
    ...
    shareInfo.sid = g_sourceInfo.sid;
    ...
    try {
      ...
      if (result === "success") {
        notify.info(gs("action:emailsent", "Email Sent!"));
      }
      else {
...
function makeMaillistSelect(maillist) {
  ...
  return new Promise((resolve, reject) => {
    let listnames = getString("label:listnames", "Mail List Names");
    let selectaddress = getString("label:selectaddress", "Select Email Address(es)");
