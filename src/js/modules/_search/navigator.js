...
class PageMatches {
  ...
  setState(location, remove) {
    if (remove) {
      $(".search-navigator .remove-match").html(`<i class="trash restore red icon"></i>${gs("srch:s39", "Restore")}`);
      $(`#${location}`).addClass("remove");

      $(".search-navigator .save-changes.button").removeClass("disabled");

      //$(".remove-count").text(` (${this.removeCount} ${gs("srch:s29", "matches marked")})`);
      $(".remove-count").text(` (${this.removeCount})`);
    }
    else {
      $(".search-navigator .remove-match").html(`<i class="trash green icon"></i>${gs("srch:s38", "Remove")}`);
  ...
  setTitle() {
    ...
    let sf = gs("srch:s9", "Search for");
    let of = gs("srch:s15", "of");
...
function initClickListeners(matches) {
  ...
  $(".search-navigator .close-window").on("click.navigator", function(e) {
    ...
    if (matches.itemsMarkedForRemoval()) {
      notify.error(gs("srch:s30", "Save changes before closing Navigator."));
      return;
    }
  ...
  $(".search-navigator a.remove-hit-check").on("click.navigator", function(e) {
    ...
    if (matches.itemsMarkedForRemoval()) {
      notify.error(gs("srch:s31", "Save changes before leaving page."));
      return false;
    }
  ...
  $(".search-navigator .save-changes.button").on("click.navigator", function(e) {
    ...
    if (matches.removeCount > 1) {
      notify.info(`${matches.removeCount} ${gs("srch:s32", "Matches Removed")}!`);
    }
    else {
      notify.info(`1 ${gs("srch:s33", "Match Removed")}!`);
    }
...
function initControls(pid) {
  ...
  if (!lastSearch) {
    notify.warning(gs("srch:s34", "There are no search results to show."));
    return;
  }
  ...
  if (!lastSearch.pageInfo[pageKey]) {
    notify.warning(`${gs("srch:s35", "There is no search result at")} ${pid}`);
    return;
  }
  ...
  if (markFail) {
    notify.info(`${gs("srch:s36", "Failed to highlight")} ${markFail} ${gs("srch:s37", "search results")}`);
  }
