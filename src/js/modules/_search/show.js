...
function addEditToggle(html, count) {
  ...
  return `<button class="ui disabled primary button save-modified-search-list">
            ...
            ${gs("srch:s1","Save Changes")}
          </button>
          <div class="ui toggle edit checkbox">
            <input type="checkbox" name="public">
            <label>${gs("srch:s2", "Select Items to Remove")}</label>
          </div>
          ${html}`;
}
...
export async function showSearchResults(data, query) {
  ...
  try {
    ...
    console.error(err);
    notify.error(gs("srch:s3","Error displaying search results, see console for details."));
  }
}
...
function plural(count) {
  if (count > 4) return `${gs("srch:s8", "matches")}`;

  if (count > 2) return `${gs("srch:s40", "matches")}`;

  if (count > 1) return `${gs("srch:s29", "matches")}`;

  return `${gs("srch:s28", "match")}`;
}
...
export function showSavedQuery() {
  ...
  if (queryResult.uniqueId) {
    $(".search-message.header").text(gs("srch:s4","Saved Search Result"));
    $(".search-message-body").html(`<p>${gs("srch:s5","Saved Search")} <em>${queryResult.query}</em> ${gs("srch:s6", "has")} ${queryResult.count} ${plural(queryResult.count)}</p>`);
  }
  else {
    $(".search-message.header").text(gs("srch:s10", "Last Search Result"));
    $(".search-message-body").html(`<p>${gs("srch:s9", "Search for")} <em>${queryResult.query}</em> ${gs("srch:s7", "found")} ${queryResult.count} ${plural(queryResult.count)}</p>`);
  }
