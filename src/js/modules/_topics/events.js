  ...
  if (q.url === "nolink") {
    ...
    html = `
      <blockquote>
        ${quote}
        <footer>
          <a href="${q.url}" title="${gs("quote:g2s", "Go to the source")}" target="_blank">
            ~ ${q.citation}
          </a>
        </footer>
      </blockquote>
...
function loadQuotes(qm, si, userInfo) {
  let systemQuoteIds = getQuoteIds(si.quoteManagerId, si.sourceId);

  if (userInfo && userInfo.userId !== si.quoteManagerId) {
    let userQuoteIds = getQuoteIds(userInfo.userId, si.sourceId);

    Promise.all([systemQuoteIds, userQuoteIds]).then((responses) => {
      let q0 = new RandomQuotes(si.quoteManagerName, si.quoteManagerId, si.sourceId); 
      q0.qIds = responses[0];
      qm.addQuotes(q0);

      if (responses[1].length > 0) {
        //let q1 = new RandomQuotes(userInfo.name, userInfo.userId, si.sourceId); 
        let q1 = new RandomQuotes("My Quotes", userInfo.userId, si.sourceId); 
        q1.qIds = responses[1];
        qm.addQuotes(q1);

        //use system quotes to start
        qm.use(si.quoteManagerId);
    ...
    systemQuoteIds.then((ids) => {
      let q = new RandomQuotes(si.quoteManagerName, si.quoteManagerId, si.sourceId); 
      q.qIds = ids;
      qm.addQuotes(q);
    });
  }
...
export function initQuoteDisplay(selector, si) {
